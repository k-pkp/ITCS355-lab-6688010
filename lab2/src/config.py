...
class Config:
    ...
    def raw_path(self) -> Path:
        """Where the raw dataset lives. Derived, never hard-coded by a caller."""
...
def load(strict: bool = True) -> Config:
    """Build the Config from the environment, failing early on a missing slot."""
    missing = [slot_name for slot_name in CAPABILITY_SLOTS if not os.environ.get(slot_name)]
