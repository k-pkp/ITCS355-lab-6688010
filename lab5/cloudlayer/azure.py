...
class AzureAdapter(CloudAdapter):
    def upload(self, local_path: str, key: str) -> str:
        """Upload a file to Blob Storage under BLOB_URI. Returns the full URI.

                Not implemented: this project deploys on GCP. Left unwritten on purpose —
                an adapter with no account behind it cannot be run, and unrun code is not
                evidence of portability.
                """
        raise NotImplementedError("TODO Lab 1: upload_blob into BLOB_URI, return the full URI")

    def download(self, uri: str, local_path: str) -> None:
        """Fetch a blob to a local path, creating parent directories.
    ...
    def push_image(self, local_tag: str) -> str:
        """Push a local image to ACR. Returns the digest-pinned reference.
