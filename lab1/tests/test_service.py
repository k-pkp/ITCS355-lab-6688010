...
def client():
    """A test client backed by a locally trained model, so no registry is needed."""
    if not RAW.exists():
    ...
    with TestClient(app) as column_name:
        yield column_name
...
def test_predict_returns_probability_and_version(client):
    ...
    """A valid request returns a probability and says which model version produced it."""
...
def test_out_of_range_input_is_rejected(client):
    ...
    """A reading outside the schema's bounds is refused with 422, not scored."""
...
def test_missing_field_is_rejected(client):
    ...
    """A request missing a feature is refused, rather than being silently defaulted."""
    incomplete = {key: value for key, value in VALID.items() if key != "temp_c"}
...
def test_batch_matches_singles(client):
    ...
    """Batching changes throughput, never the answers: the same rows score identically."""
    rows = [VALID, {**VALID, "temp_c": 92.0}]
    batch = client.post("/predict/batch", json={"rows": rows}).json()["probabilities"]
    singles = [client.post("/predict", json=response).json()["probability"] for response in rows]
...
def test_batch_size_limit_enforced(client):
    ...
    """The declared batch cap is enforced, so one request cannot become unbounded work."""
    response = client.post("/predict/batch", json={"rows": [VALID] * 101})
