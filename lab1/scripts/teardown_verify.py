...
def main() -> int:
    """Look for resources still carrying this lab's tags, and report what remains."""
    argument_parser = argparse.ArgumentParser()
    argument_parser.add_argument("--lab", type=int, required=True)
    options = argument_parser.parse_args()

    loaded_config = config.load()
    tags = loaded_config.tags(options.lab)
    adapter = get_adapter(loaded_config)
