...
def load_raw(path: Path) -> pd.DataFrame:
    """Read the raw sensor readings, or explain how to obtain them."""
    if not path.exists():
...
def split(
    ...
    n_test = int(round(group_count * test_frac))
    n_val = int(round(group_count * val_frac))
    ...
    parts = tuple(
        frame[frame[GROUP].isin(group_ids)].sort_values(IDENTIFIER).reset_index(drop=True)
        for group_ids in (train_g, val_g, test_g)
    )
    if any(len(partition) == 0 for partition in parts):
